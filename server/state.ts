...
const INITIAL_ZONES: StadiumZone[] = [
  ...
  },
  {
    ...
    density: 68, // Red zone! High density triggers agents
    capacity: 9000,
    currentCount: 6120,
    status: 'HIGH',
    queueTimeMin: 25,
  ...
  },
  {
    ...
    density: 76, // Red zone! High density
    capacity: 9000,
    currentCount: 6840,
    status: 'CRITICAL',
    queueTimeMin: 32,
  ...
  agentLogs: [
    {
      ...
      message: 'System started. Initialized stadium twin for FIFA World Cup 2026. Monitoring all 12 operational zones.',
      status: 'COMPLETED'
    }
  ]
};

// Calculate status category from percentage density
function getStatusFromDensity(density: number): 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' {
...
// Predict prep quantities based on zone density
export function generateVendorPreps() {
  state.vendorPreps = state.zones.map(zone => {
...
// Trigger Multi-Agent Safety mitigation workflow when a zone surpasses 55%
function runMultiAgentSafetyCheck(zoneId: string, oldDensity: number, newDensity: number) {
  const zone = state.zones.find(z => z.id === zoneId);
  if (!zone) return;

  // Trigger conditions
  if (newDensity >= 55 && oldDensity < 55) {
    const timestamp = new Date().toLocaleTimeString();

    // 1. Supervisor Agent logs and delegates
    const supervisorLog: AgentLog = {
      id: `agent-log-${Date.now()}-1`,
      timestamp,
      agentName: 'Supervisor Agent',
      message: `🚨 ALERT: Density in ${zone.name} surged to ${newDensity}%. Delegating turnstile and crowd flow analysis immediately.`,
      status: 'PROCESSING'
    };
    state.agentLogs.unshift(supervisorLog);

    // Find adjacent gates for rerouting
    const adjacentGates = state.zones
      .filter(z => z.id !== zoneId && z.id.startsWith('zone-') && z.id.length === 6 && z.density < 45)
      .slice(0, 2)
      .map(z => z.name)
      .join(' and ');

    const alternateText = adjacentGates || 'Gates B and D';

    // 2. Analysis Agent scans turnstiles & gates
    setTimeout(() => {
      const timestamp2 = new Date().toLocaleTimeString();
      const analysisLog: AgentLog = {
        id: `agent-log-${Date.now()}-2`,
        timestamp: timestamp2,
        agentName: 'Analysis Agent',
        message: `🔍 ANALYSIS: Turnstile flow rate at ${zone.name} is heavy (${Math.round(zone.currentCount * 0.04)} entry events/min). Alternative entry corridors ${alternateText} verified below 45% capacity.`,
        status: 'PROCESSING'
      };
      state.agentLogs.unshift(analysisLog);

      // update current log status
      const targetSup = state.agentLogs.find(l => l.id === supervisorLog.id);
      if (targetSup) targetSup.status = 'COMPLETED';

      // 3. Mitigation Agent dispatches plan
      setTimeout(() => {
        const timestamp3 = new Date().toLocaleTimeString();
        const mitigationLog: AgentLog = {
          id: `agent-log-${Date.now()}-3`,
          timestamp: timestamp3,
          agentName: 'Mitigation Agent',
          message: `⚙️ MITIGATION: Broadcaster triggered! Digital twin signage at ${zone.name} updated to: "USE ALTERNATIVE CORRIDORS: ${alternateText.toUpperCase()}". Operational safety margin recovered.`,
          status: 'ALERT'
        };
        state.agentLogs.unshift(mitigationLog);

        const targetAnalysis = state.agentLogs.find(l => l.id === analysisLog.id);
        ...
        const newIncident: Incident = {
          ...
          zoneId: zone.id,
          zoneName: zone.name,
          severity: newDensity >= 75 ? 'CRITICAL' : 'WARNING',
          message: `Autonomous agents mitigated high crowd load (${newDensity}%) at ${zone.name}. Dynamic signage routed flow to ${alternateText}.`,
          status: 'RESOLVING',
          agentOwner: 'Mitigation Agent',
          mitigationPlan: `Interactive digital banners updating to disperse crowds to ${alternateText}. Queue wait reduced.`
        };
        state.incidents.unshift(newIncident);

        // Disperse some crowd after mitigation (simulating agent action)
        setTimeout(() => {
          zone.density = Math.max(45, zone.density - 12);
          zone.currentCount = Math.round(zone.capacity * (zone.density / 100));
          zone.status = getStatusFromDensity(zone.density);
          zone.queueTimeMin = Math.max(5, Math.round(zone.density * 0.4));
          generateVendorPreps();
          
          const resolveLog: AgentLog = {
            id: `agent-log-${Date.now()}-4`,
            timestamp: new Date().toLocaleTimeString(),
            agentName: 'Supervisor Agent',
            message: `✅ RESOLUTION: Density at ${zone.name} reduced to ${zone.density}%. Safety threshold stabilized successfully. Closing multi-agent operation.`,
            status: 'COMPLETED'
          };
          state.agentLogs.unshift(resolveLog);

          const incident = state.incidents.find(inc => inc.zoneId === zone.id && inc.status === 'RESOLVING');
          if (incident) {
...
// Initial calculation for prep
generateVendorPreps();

export function getStadiumState(): StadiumState {
  return state;
}

export function updateZoneDensity(zoneId: string, density: number) {
  const zone = state.zones.find(z => z.id === zoneId);
  if (zone) {
    const oldDensity = zone.density;
    zone.density = Math.max(0, Math.min(100, density));
    zone.currentCount = Math.round(zone.capacity * (zone.density / 100));
    zone.status = getStatusFromDensity(zone.density);
    // Queue time increases non-linearly with density
    zone.queueTimeMin = Math.round(zone.density * 0.4);

    // Reactively trigger Multi-Agent Safety
    runMultiAgentSafetyCheck(zoneId, oldDensity, zone.density);

    // Regenerate preps to match
    generateVendorPreps();
  }
}
  ...
  state.agentLogs = [
    {
      ...
      timestamp: new Date().toLocaleTimeString(),
      agentName: 'Supervisor Agent',
      message: 'System state reset. Stadium digital twin fully synchronized for World Cup 2026.',
      status: 'COMPLETED'
    }
  ];
  generateVendorPreps();
...
export function addIncident(zoneId: string, severity: 'INFO' | 'WARNING' | 'CRITICAL', message: string) {
  const zone = state.zones.find(z => z.id === zoneId);
  const name = zone ? zone.name : 'Unknown Zone';
  const newIncident: Incident = {
    id: `inc-${Date.now()}`,
    timestamp: new Date().toLocaleTimeString(),
    zoneId,
  ...
  // If critical, raise Supervisor warning immediately
  if (severity === 'CRITICAL' || severity === 'WARNING') {
    state.agentLogs.unshift({
      id: `agent-log-${Date.now()}`,
      timestamp: new Date().toLocaleTimeString(),
      agentName: 'Supervisor Agent',
      message: `⚠️ OPERATOR ALERTER: Incident flagged in ${name}: "${message}". System dispatching mitigation recommendations.`,
      status: 'ALERT'
    });
  }
}
